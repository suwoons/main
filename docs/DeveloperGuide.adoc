...
User can choose to snooze the reminder in any/all of the following time units: days, hours and/or minutes. Only positive integer are allowed as the user are not supposed to snooze the reminders backwards or snooze it by zero duration. Following is an activity digram for the command.
...
==== Implementation
The key command which users can call is the `calendar` command.

The following sequence diagram demonstrates how the `calendar` command works, as a `Calendar` window is displayed to the user.
