...
public class NoteModCommandParser implements Parser<NoteModCommand> {
    ...
    public NoteModCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
            ArgumentTokenizer.tokenize(args, PREFIX_MODULE_NOTE);

        Index index;
        try {
            index = ParserUtil.parseIndex(argMultimap.getPreamble());
        } catch (ParseException pe) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, NoteModCommand.MESSAGE_USAGE), pe);
        }

        if (!arePrefixesPresent(argMultimap, PREFIX_MODULE_NOTE)
        ...
        return new NoteModCommand(index, note);
    }
